...
const Transfer = ({
  ...
  // IMAGE URL WHEN SELECTED TOKEN CHANGES
  useEffect(() => {
  ...
  // UPDATE BALANCE WHEN ACTIVE TOKEN OR WALLET CHANGE
  useEffect(() => {
  ...
  const sendCrypto = async (amount, recipient) => {
    ...
    if (activeThirdWebToken && amount && recipient) {
      const tx = await activeThirdWebToken.transfer(
        recipient,
        amount.toString().concat("000000000000000000") // 18 decimals tokens needed
      );
      console.log(tx);
